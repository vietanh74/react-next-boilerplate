...
import { IResponse, ExtendedAxiosInstance, ExtendedAxiosRequestConfig } from '../../interfaces';
import { authService } from '@/services';
import { jsonDecode } from '@/helpers';
import { getAppAccessToken, removeAppToken, setAppToken } from '@/core/auth';
import { isNotifyWhenFail } from './helpers';
...
export class BaseClient {
  ...
  private transformResponse(res: AxiosResponse): IResponse | AxiosResponse {
    if (this.noTransform || (res.config as ExtendedAxiosRequestConfig).noTransform) {
      return res;
    }
      ...
      async (error: AxiosError) => {
        ...
        if (this.isRefreshing) {
          return new Promise((resolve, reject) => {
            ...
            .then((token) => {
              return (api as AxiosInstance)(originalRequest);
            })
            .catch((err) => {
        ...
        return Promise.resolve((api as AxiosInstance)(originalRequest));
      },
    );
